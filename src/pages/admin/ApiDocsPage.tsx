      ...
      <div className="min-h-screen p-8 landscape:px-16 text-white">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <p className="text-white/95 text-lg">{t('admin.api.subtitle')}</p>
          </motion.div>

          <Card className="p-6 bg-white/95 backdrop-blur-md border-2 border-white/30 mb-6">
            <div className="flex items-center gap-3 mb-4">
              <Terminal className="w-6 h-6 text-green-600" />
              <h2 className="text-2xl font-bold text-gray-800">{t('admin.api.graphqlEndpoint')}</h2>
            </div>
            <code className="block p-4 bg-gray-100 rounded-lg text-gray-800 mb-4">
              POST http://localhost:5000/graphql
            </code>
            <p className="text-gray-700 mb-4">
              {t('admin.api.endpointDescription')} http://localhost:5000/graphql
            </p>
            <Button
              onClick={() => window.open('http://localhost:5000/graphql', '_blank')}
              className="bg-green-600 hover:bg-green-700 text-white"
            >
              {t('admin.api.openPlayground')}
            </Button>
          </Card>

          <Card className="p-6 bg-white/95 backdrop-blur-md border-2 border-white/30">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('admin.api.mainQueries')}</h2>
            <div className="space-y-4">
              <div>
                <h3 className="font-bold text-gray-800 mb-2">{t('admin.api.authentication')}</h3>
              ...
              <div>
                <h3 className="font-bold text-gray-800 mb-2">{t('admin.api.companyAnalytics')}</h3>
                <code className="block p-3 bg-gray-100 rounded text-sm text-gray-800">
                  query GetAnalytics($companyId: String!) {'{'} companyAnalytics(companyId: $companyId) {'{'} binUsage leaderboard {'}'} {'}'}
                </code>
              </div>
              <div>
                <h3 className="font-bold text-gray-800 mb-2">{t('admin.api.createEmployee')}</h3>
                <code className="block p-3 bg-gray-100 rounded text-sm text-gray-800">
